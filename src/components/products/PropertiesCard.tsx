  ...
  return (
    <>
      <div className="flex justify-between h-96 shadow-sm">
        <div className="bg-neutral-50 border border-neutral-300 rounded pt-3 px-10 overflow-y-scroll card w-full dark:bg-neutral-900 dark:border-neutral-800">
          <div className="flex items-center justify-between sticky top-0 bg-neutral-100 border border-neutral-300 px-3 py-1 rounded-md z-20 dark:bg-neutral-900 dark:border-neutral-800">
            <h3 className="font-semibold text-neutral-700 dark:text-neutral-500">
              {props.title}
            </h3>

            <button onClick={close} type="button"
              className="bg-neutral-200 border border-neutral-300 flex items-center justify-between gap-2 w-52 -mr-2 py-1 rounded font-medium text-sm group duration-75
              hover:bg-accent hover:text-white hover:border-neutral-800 dark:bg-neutral-900 dark:border-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-800">
              <span className="mx-auto">
                Adicionar {props.title.toLowerCase()}
              </span>
              <img src={externalicon} alt="" className="group-hover:brightness-[6] -ml-10 mr-3 w-4 -scale-x-100 duration-75 dark:brightness-[6]" />
            </button>
          </div>

          <div>
            <table className="w-full border-separate border-spacing-y-2">
              <thead className="text-left bg-accent text-white sticky top-12 z-10 dark:text-neutral-400 dark:bg-neutral-800">
                <tr>
              ...
              <tbody className="[&>*:nth-child(even)]:bg-neutral-100 dark:[&>*:nth-child(even)]:bg-neutral-915">
      ...
      {showAddProperty && (
        <div className="absolute top-0 left-0 w-full bg-neutral-950 bg-opacity-20 z-50">
          <div className="flex flex-col justify-center h-screen mx-auto w-fit">
            <div className="bg-white rounded border border-neutral-300 px-12 py-9 w-[800px] h-[400px] relative shadow-sm dark:bg-neutral-900 dark:border-neutral-800">
              ...
              <h4 className='px-7 mb-6 font-medium text-sm text-neutral-500 dark:text-neutral-600'>
                VOCÊ ESTÁ ADICIONANDO <span className="font-bold text-red-400 dark:text-red-500">{props.title.toUpperCase()}</span>!
              </h4>
              <form className="space-y-4 px-7" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center justify-end absolute right-0 bottom-0 p-4">
                  <button className="mx-5 my-2 dark:text-neutral-400" type="button" onClick={close}>
                    Cancelar
                  </button>
                  <button disabled={!name}
                    className="rounded px-5 py-1.5 text-white bg-accent disabled:bg-neutral-100 border
                    disabled:border-neutral-300 disabled:text-neutral-500 disabled:cursor-not-allowed
                    dark:bg-accent-dark dark:text-neutral-800
                    dark:disabled:bg-neutral-900 dark:disabled:border-neutral-800 dark:disabled:text-neutral-700">
                    Salvar
                  </button>
                </div>
              </form>
